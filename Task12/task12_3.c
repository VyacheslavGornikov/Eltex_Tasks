...
int main(void) 
{
    ...
    while (1) 
    {
        mvprintw(y_max - 1, 0, "Press q to quit, W - up, S - down, Tab - switch window, c - execute command");
        refresh();
        ...
        else if (ch == 'c') // ввод команды с клавиатуры
        {
            input_command(command, active_window);
            if (active_window == 1) 
            {
                execute_command(command, files, &num_files, path1);         
                wclear(wnd1);
                print_box(wnd1);
                selected1 = 0;  
            }
            ...
            {
                execute_command(command, files, &num_files, path2);         
                wclear(wnd2);
                print_box(wnd2);
                selected2 = 0; 
            }
